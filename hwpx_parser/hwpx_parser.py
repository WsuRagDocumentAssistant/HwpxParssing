...
from .parser.meta_data_parser import MetadataParser
from .parser.section_data_parser import SectionParser
...
class HwpxParser:
    ...
    def __ensure_unpacked(self) -> None:
        ...
        # 기본 위치에 content.hpf가 없으면 압축 해제 폴더 전체에서 content.hpf 탐색
        # zip 내부에 문서 폴더가 한 겹 더 들어간 경우를 처리하기 위한 코드
        if not self.__METADATA_FILE_PATH.exists():
            found_metadata_files = list(self.__UNPACKED_DIR_PATH.rglob("content.hpf"))

            if not found_metadata_files:
                raise FileNotFoundError(
                    f"content.hpf를 찾을 수 없습니다: {self.__UNPACKED_DIR_PATH}"
                )

            # content.hpf 중 첫 번째 파일을 선택
            metadata_file_path = found_metadata_files[0]

            # content.hpf가 있는 폴더를 Contents 폴더로 다시 설정
            self.__CONTENTS_DIR_PATH = metadata_file_path.parent
            # section*.xml 파일 경로를 다시 설정
            self.__METADATA_FILE_PATH = metadata_file_path
            # header.xml 파일 경로를 다시 설정
            self.__HEADER_FILE_PATH = self.__CONTENTS_DIR_PATH / "header.xml"
            # 이미지 파일 경로를 다시 설정
            self.__IMGS_DIR_PATH = self.__CONTENTS_DIR_PATH.parent / "BinData"
